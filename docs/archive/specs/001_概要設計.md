...
### UIアーキテクチャ（SwiftUI移行フェーズ1）
- アプリエントリポイントは SwiftUI の `PoseFinderApp`（`@main`）とし、`@UIApplicationDelegateAdaptor` で既存 `AppDelegate` を併用する。
- `PoseFinderApp` から `LegacyRootView`（`UIViewControllerRepresentable`）を介して Storyboard 初期 ViewController を起動し、既存 UIKit 画面群をそのまま利用する。
- SwiftUI 側でナビゲーションレイヤーを段階的に導入できるよう、今後の画面差し替えを `LegacyRootView` 配下で進める。
- フェーズ2では SwiftUI `HomeView` / `TrainingMenuDetailView` と、そのデータ供給を担う `HomeViewModel` / `TrainingMenuDetailViewModel` を導入し、メニュー選択から UIKit 録画画面への橋渡しを行う。
...
2. **ホーム画面(トレーニングメニュー一覧)**
   - SwiftUI `HomeView`
   - 利用可能なトレーニングメニューの一覧（ダミーデータから開始）
   - （将来）画像 + メニュー名 + お気に入り等のメタ情報

3. **トレーニングメニュー詳細**
   - SwiftUI `TrainingMenuDetailView`
   - セッション一覧（履歴）の表示
   - 「お手本再生(仮)」ボタン
   - 「撮影開始」ボタン
   - SwiftUI から既存 UIKit 録画画面 (`RecordingSessionContainerView` → `ViewController`) へ遷移
...
5. **セッション詳細画面**
   - 各セッションをタップしたときに開く画面
   - SwiftUI `SessionDetailView`（AVPlayer + Pose プレビュー）
   - 保存されたVideo再生コンポーネント
   - Pose NDJSON のプレビュー描画（将来、再生同期へ拡張予定）
   - 再生時のオーバーレイ表示ON/OFF切替（今後の対応）
